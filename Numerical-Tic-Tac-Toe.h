...
void Numerical_Tic_Tac_Toe_Player<T>::getmove(int& x, int& y) {
    ...
    while (true) {
        ...
        cin >> x >> y;
        if (x < 0 || x > 2 || y < 0 || y > 2) {
            cout << "Invalid move! Both x and y must be between 0 and 2. Please try again." << endl;
            continue;
        }
        // Generate the position string in the format "xy"
        position = to_string(x) + to_string(y);
        // Check if the position is in the vector
        auto it = find(positions.begin(), positions.end(), position);
        if (it != positions.end()) {
            // Valid position, remove it from the vector
            positions.erase(it);
            break;
        } else {
...
int  Numerical_Tic_Tac_Toe_board<T>::get_num(int x) {
    int num;
    if (choice3 == 2 && choice4 == 1 || choice4==2 && choice3==1) {
        if (p) {
